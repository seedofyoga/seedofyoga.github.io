...
describe("E2E — Mobile menu", () => {
  it("toggles mobile menu on hamburger click", async () => {
    ...
    const expandedAfterOpen = await page.$eval("#menu-toggle", (el) =>
    ...
    const expandedAfterClose = await page.$eval("#menu-toggle", (el) =>
      el.getAttribute("aria-expanded"),
    );
  ...
  it("closes mobile menu when a nav link is clicked", async () => {
    ...
    if (link) {
      await link.click();
      await waitForAria(page, "false");
      const expanded = await page.$eval("#menu-toggle", (el) =>
        el.getAttribute("aria-expanded"),
      );
