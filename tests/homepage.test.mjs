...
describe("Homepage — Navigation", () => {
  ...
  for (const [text, href] of [
    ...
    it(`has "${text}" link pointing to ${href}`, () => {
      const link = navLinks.filter(
        (_, el) => $(el).text().trim() === text,
      );
    ...
    for (const day of schedule.days) {
      for (const cls of day.classes) {
        ...
        assert.ok(
          ...
          `should contain class "${cls.name}"`,
        );
        assert.ok(
          html.includes(cls.time),
          `should contain time "${cls.time}"`,
        );
      }
    }
  });
...
describe("Homepage — Pricing", () => {
  ...
  it("renders all plan names and prices", () => {
    ...
    for (const plan of pricing.plans) {
      assert.ok(
        html.includes(plan.name),
        `should contain plan "${plan.name}"`,
      );
      assert.ok(
        html.includes(String(plan.price)),
        `should contain price "${plan.price}"`,
      );
    }
  });
...
describe("Homepage — Blog Preview", () => {
  ...
  it('has "View All Posts" link', () => {
    const link = $('a').filter((_, el) =>
      $(el).text().includes("View All Posts"),
    );
